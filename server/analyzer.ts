...
import { deductCreditsAtomic, refundCredits, CREDIT_COSTS } from './security/creditManager.js';
...
export async function analyzeWebsitePipeline(
  ...
  const ownerId = options?.ownerId || 'guest_default';
  ...
  // 2. Cache Lookup Check (Instant diagnosis without consuming credits)
  ...
  const userSlotAcquired = ConcurrencyGate.tryAcquireUserSlot(ownerId);
  if (!userSlotAcquired) {
    const err = new Error('Maximum concurrent website analyses reached for your account. Please wait for current analysis to finish.');
    (err as any).stage = 'CONCURRENCY_LIMIT';
    throw err;
  }

  // 4. Server-Side Atomic Credit Deduction
  const creditDeduction = deductCreditsAtomic(ownerId, 'ANALYSIS', options?.idempotencyKey || recordId);
  if (!creditDeduction.success) {
    ConcurrencyGate.releaseUserSlot(ownerId);
    const err = new Error(creditDeduction.error || 'Insufficient credits for website analysis.');
    (err as any).stage = 'CREDIT_CHECK';
    (err as any).isCreditError = true;
    throw err;
  }

  try {
    // 5. Create Analysis Record with unique ID and owner association
    ...
    const initResult = await createAnalysisRecord(rawUrl, cleanUrl, ownerId, recordId);
    timings.dbPersistence = Date.now() - dbInitStart;
    if (initResult.persistedToCloud) {
      console.log(`[REVO][${recordId}] stage=SUPABASE_INITIALIZATION SUCCESS (${timings.dbPersistence}ms)`);
    } else {
      console.log(`[REVO][${recordId}] stage=SUPABASE_INITIALIZATION FAILED (cloudStatus=${initResult.cloudReason || 'TABLE_MISSING_OR_UNAVAILABLE'}; in-memory storage active)`);
    }

    // 6. Hard Overall Deadline Execution Wrapper
    ...
    const executionPromise = (async () => {
      ...
      while (attempt < REVO_CONFIG.RETRY.MAX_ATTEMPTS) {
        ...
        try {
          ...
          const updateResult = await updateAnalysisStatus(recordId, 'completed', {
            site: {
              title: evidence.title,
              description: evidence.metaDescription,
          ...
          if (updateResult.persistedToCloud) {
            console.log(`[REVO][${recordId}] stage=SUPABASE_PERSISTENCE SUCCESS (${Date.now() - persistenceStart}ms)`);
          } else {
            console.log(`[REVO][${recordId}] stage=SUPABASE_PERSISTENCE FAILED (cloudStatus=${updateResult.cloudReason || 'TABLE_MISSING_OR_UNAVAILABLE'}; in-memory record updated)`);
          }
    ...
    try {
      ...
      // Refund credits if failure occurred before AI reasoning phase consumed resources
      if (!creditDeduction.reused && (failedStage === 'URL_VALIDATION' || failedStage === 'SUPABASE_INITIALIZATION' || failedStage === 'EVIDENCE_EXTRACTION' || (err as any)?.isSsrfViolation)) {
        refundCredits(ownerId, CREDIT_COSTS.ANALYSIS, `Analysis failed before AI pass at stage: ${failedStage}`);
      }
